...
export function AlertZoneStatusSection() {
    ...
    const zoneTypes = [
        ...
        { key: 'frequent', label: 'Zona Frecuente', icon: MapPin, color: 'text-amber-500', bgColor: 'bg-amber-500/10' }
    ];
                        ...
                        <div key={type.key} className="flex items-center justify-between p-3 rounded-xl bg-dark-bg/50 border border-white/5">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <div className="text-[10px] text-muted-foreground uppercase tracking-wider">
                                        {zone ? 'Configurado' : 'Sin configurar'}
                                    </div>
                                </div>
                            </div>
                            ...
                            {!zone ? (
                                <Link to="/explorar" state={{ activateZoneType: type.key }}>
                                    <Button variant="ghost" size="sm" className="h-8 text-neon-green hover:text-neon-green hover:bg-neon-green/10 gap-1 text-xs">
                                        Configurar
                                        <ChevronRight className="w-3 h-3" />
                                    </Button>
                                </Link>
                            ) : (
                                <div className="text-[10px] text-neon-green font-bold bg-neon-green/10 px-2 py-1 rounded">
