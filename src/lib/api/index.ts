...
import { createGeocodeApi } from './domains/geocode';
import { createUserZonesApi } from './domains/user-zones';
import { createGamificationApi } from './domains/gamification';
...
export async function apiRequest<T>(
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 401 && sessionAuthority.getToken()?.jwt) {
        forceLogout('SESSION_EXPIRED');
      }

      throw new AppClientError(
        data.message || data.error || `HTTP ${response.status}: ${response.statusText}`,
        response.status === 401 ? 'SESSION_EXPIRED' : (data.code || 'HTTP_ERROR'),
        response.status,
    ...
    if (error instanceof AppClientError) {
      if (error.statusCode === 401 && sessionAuthority.getToken()?.jwt) {
        forceLogout('SESSION_EXPIRED');
      }
...
export const geocodeApi = createGeocodeApi(apiRequest);
...
export const userZonesApi = createUserZonesApi<UserZone, UserZoneData>(apiRequest);
...
export const gamificationApi = createGamificationApi(apiRequest);

// ============================================
// FAVORITES API
// ============================================

export const favoritesApi = createFavoritesApi(apiRequest);
...
export const seoApi = createSeoApi<ZoneSEO>(apiRequest);
